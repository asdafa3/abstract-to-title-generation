...
from trl.trl.core import (logprobs_from_logits,
                         whiten,
                         clip_by_value,
                         entropy_from_logits,
                         flatten_dict,
...
class PPOTrainer:
    ...
    def __init__(self, model, ref_model, v_head, **ppo_params):
        """
    ...
    def batched_forward_pass(self, model_input, gen_len):
        ...
        for i in range(int(self.ppo_params['batch_size']/fbs)):
            ...
            #encoder_last_hidden_state = self.model(m_input).encoder_last_hidden_state
            #v = self.v_head(encoder_last_hidden_state)
            last_decoder_hidden_states = self.model(m_input).decoder_hidden_states[-1] 
            v = self.v_head(last_decoder_hidden_states)
    ...
    def loss(self, old_logprobs, values, rewards, query, response, model_input):
        ...
        hs = self.model(model_input).encoder_last_hidden_state
        vpred = self.v_head(hs)
